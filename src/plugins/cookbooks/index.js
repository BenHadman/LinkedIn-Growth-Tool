...
function pluginContentCookbook(context, opts) {
  ...
  const pluginDataDirRoot = path.join(generatedFilesDir, "cookbooks");
  ...
  return {
    ...
    async loadContent() {
      const guidePosts = await generateCookbookPosts(
        contentPath,
        context,
        options
      );
